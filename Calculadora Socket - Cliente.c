...
int main(){
    ...
    if(WSAStartup(MAKEWORD(2, 2), &winsock) != 0) {
        printf("\nFalha na inicializaçãoo da biblioteca winsock!\n");
        return(1);
    }
    ...
    do {
        ...
        //Aguardando mensagem solicitando o primeiro número.
        if(recv(socket_aplicacao, mensagem, T_MAX, 0) <= 0){
            printf("\nFalha ao receber a messagem do servidor. Erro: %d\n", WSAGetLastError());
            closesocket(socket_aplicacao);
            WSACleanup();
        ...
        //Aguardando mensagem solicitando o segundo número.
        if(recv(socket_aplicacao, mensagem, T_MAX, 0) <= 0){
            printf("\nFalha ao receber a messagem do servidor. Erro: %d\n", WSAGetLastError());
            closesocket(socket_aplicacao);
            WSACleanup();
